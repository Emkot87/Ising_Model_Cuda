...
__global__ void Ising(int* G, int* L,int n, int threads,int blocks,int bSize){
  ...
  int blockIndex = blockIdx.x * threads;
  int ku = blockIndex/s * bSize;
  int fu = blockIndex%s * bSize;
  ...
  extern __shared__ int blockMem[];
    ...
    for(int j = (f * bSize) ; j < (f+1) * bSize && j < n; j++ ){
      blockMem[(i - ku + 1)*(bSize*threads) + j - fu] = G[i*n+j]; 
    }
  }

	// values above
	for(int j = (f * bSize) ; j < (f+1) * bSize && j < n; j++ ){
		blockMem[j - fu] = G[(k*bSize - 1 + n)%n*n + j];
	}

	// values below
	for(int j = (f * bSize) ; j < (f+1) * bSize && j < n; j++ ){
		blockMem[bSize*threads*(bSize+1) + j - fu] = G[((k+1)*bSize)%n*n + j];
	}
	...
	if(threadIdx.x == 0){
		...
		for(int i = (k * bSize) ; i < (k+1) * bSize && i < n; i++){
    	for(int j = (f * bSize) + 1 ; j < (f+1) * bSize && j < n; j++ ){
				int x = blockMem[(i-1-ku +1)*bSize*threads + j - fu] + blockMem[(i-ku+1)*bSize*threads+(j - fu - 1)] + blockMem[(i-ku+1)*bSize*threads + j - fu] 
								+ blockMem[(i+1-ku+1)*bSize*threads + j - fu] + blockMem[(i - ku +1)*bSize*threads + (j - fu + 1)];
				L[i*n+j] = (x>0) - (x<0);
    	}
  	}
		for(int i = (k * bSize) ; i < (k+1) * bSize && i < n; i++){
    	int j = f * bSize ; 
			int x = blockMem[(i-1-ku+1)*bSize*threads + j - fu] + G[i*n+(j-1+n)%n] + blockMem[(i-ku+1)*bSize*threads + j - fu] 
							+ blockMem[(i+1-ku+1)*bSize*threads + j - fu] + blockMem[(i - ku +1)*bSize*threads + (j - fu + 1)];
			L[i*n+j] = (x>0) - (x<0);
		}
		...
		for(int i = (k * bSize) ; i < (k+1) * bSize && i < n; i++){
    	for(int j = (f * bSize) ; j < (f+1) * bSize - 1 && j < n; j++ ){
				int x = blockMem[(i-1-ku+1)*bSize*threads + j - fu] + blockMem[(i-ku+1)*bSize*threads+(j - fu - 1)] + blockMem[(i-ku+1)*bSize*threads + j - fu] 
								+ blockMem[(i+1-ku+1)*bSize*threads + j - fu] + blockMem[(i - ku +1)*bSize*threads + (j - fu + 1)];
				L[i*n+j] = (x>0) - (x<0);
    	}
    	...
    	int j = (f+1) * bSize - 1 ;
			int x = blockMem[(i-1-ku+1)*bSize*threads + j - fu] + blockMem[(i-ku+1)*bSize*threads+(j - fu - 1)] + blockMem[(i-ku+1)*bSize*threads + j - fu] 
							+ blockMem[(i+1-ku+1)*bSize*threads + j - fu] + G[i*n+(j+1)%n];
			L[i*n+j] = (x>0) - (x<0);
    }
		...
		for(int i = (k * bSize) ; i < (k+1) * bSize && i < n; i++){
    	for(int j = (f * bSize) ; j < (f+1) * bSize && j < n; j++ ){
				int x = blockMem[(i-1-ku + 1)*bSize*threads + j - fu] + blockMem[(i-ku + 1)*bSize*threads + j - fu] + blockMem[(i-ku + 1)*bSize*threads+(j - fu - 1)] + blockMem[(i+1-ku + 1)*bSize*threads + j - fu] + blockMem[(i - ku + 1)*bSize*threads + (j - fu + 1)];
				L[i*n+j] = (x>0) - (x<0);
    	}
  	}
...
int main(int argc, char* argv[]){
  ...
  for(int i = 0; i < k ; i++ ){
    Ising<<<blocks,threads,threads*(bSize+2)*bSize*sizeof(int)>>>(d_F,d_L,n,threads,blocks,bSize);
    int* temp = d_F;
    d_F = d_L;
    d_L = temp;
  }
